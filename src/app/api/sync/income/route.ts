// src/app/api/sync/income/route.ts
//
// Detecteert terugkerend inkomen per gebruiker.
// Zelfde logica als recurring detector maar voor amount > 0.
// Slaat op in merchant_map met income_hint = true.
...
function amountStability(amounts: number[]): number {
  ...
  return clamp01(1 - median(deviations) / 0.3); // inkomen mag max 30% variëren
}
...
function dayStability(days: number[]): number {
  ...
  return clamp01(1 - dev / 7); // inkomen mag max 7 dagen variëren
}
...
export async function POST(_req: NextRequest) {
  ...
  if (txs.length === 0) {
    return NextResponse.json({ success: true, income: [], updated: 0 });
  }
  ...
  for (const [k, rows] of byKey) {
    ...
    // Intervals tussen opeenvolgende transacties
    ...
    // Drempel iets lager dan uitgaven — inkomen varieert vaker (vakantiegeld, bonus)
