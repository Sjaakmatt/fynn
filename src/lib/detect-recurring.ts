...
// Kernlogica voor recurring merchant detectie.
// Geëxtraheerd uit api/sync/recurring zodat het direct callable is (geen HTTP roundtrip).
...
function cadenceFromIntervals(intervals: number[]) {
  ...
  const fitTo = (target: number, tol: number) => {
    const within = intervals.filter(d => Math.abs(d - target) <= tol).length / intervals.length
...
function amountStability(amountsAbs: number[]) {
  ...
  return clamp01(1 - median(deviations) / 0.5)
}
...
export async function detectRecurring(
  ...
  if (allTxs.length === 0) {
    return { candidates: [], updated: 0, deactivated: 0 }
  }
  ...
  for (const [k, rows] of byKey) {
    ...
    const score = clamp01(0.45 * fit + 0.3 * amtScore + 0.15 * dayScore + 0.1 * occScore)
  ...
  return { candidates, updated, deactivated: toDeactivate.length }
}